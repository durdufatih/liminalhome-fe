    ...
    useEffect(() => {
        const fetchListings = async () => {
            try {
                ...
                setListings(response.data); // API'den alınan verileri state'e kaydet
            } catch (error) {
            ...
            <div className="mt-8 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {listings.map(listing => (
                        <li key={listing._id} className="bg-blue-100 rounded-lg border border-blue-300 shadow-lg overflow-hidden">
                            <div className="p-4">
                                <h2 className="text-2xl font-bold text-blue-800">{listing.title}</h2>
                                <p className="text-blue-600">{listing.description}</p>
                            </div>
                            <div className="p-4 bg-blue-200 flex justify-between items-center">
                                <div>
                                    <p className="text-blue-700">{listing.city}</p>
                                    <p className="text-sm text-blue-600">Available from 
                                        <span className="font-semibold text-blue-900">{new Date(listing.availableDates.startDate).toLocaleDateString()}</span> to 
                                        <span className="font-semibold text-blue-900">{new Date(listing.availableDates.endDate).toLocaleDateString()}</span>
                                    </p>
                                </div>
                                <div>
                                    <button onClick={() => console.log("update")} className="text-blue-700 hover:text-blue-900 p-2">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" className="w-6 h-6">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 112.036 2.036L12 16.5V21h4.5l6.378-6.378a2.5 2.5 0 00-3.536-3.536L12 16.5l-4.5-4.5H3v-1a2 2 0 012-2h3.5l1-1m0 0L21 3m0 0L3 21" />
                                        </svg>
                                    </button>
                                    <button onClick={() => console.log("delete")} className="text-red-600 hover:text-red-800 p-2">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" className="w-6 h-6">
